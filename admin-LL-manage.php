...
<?php
		...
		if (!$conn)
	{
		...
		//==========Send Approval Comments to Initiator==========
		$conn=mysqli_connect("localhost","root","","bacs");	
		$connectDb = mysqli_select_db($conn,'bacs');
...
<?php	
		...
		if (!$conn)
	{
		...
		//==========Send Rejection Comments to Initiator==========
		$conn=mysqli_connect("localhost","root","","bacs");	
		$connectDb = mysqli_select_db($conn,'bacs');
		$query = "select * from users where username = '$submitted'";
		$result = mysqli_query($conn,$query);
...
<?php	
		...
		if (!$conn)
	{
		die('Connection error'.mysqli_connect_error);
			...
			//==========Send Rollback Comments to Initiatr==========
			$conn=mysqli_connect("localhost","root","","bacs");	
			$connectDb = mysqli_select_db($conn,'bacs');
			$query = "select * from users where username = '$submitted'";
			$result = mysqli_query($conn,$query);
            ...
            <i class="right fas fa-angle-left"></i>
            	...
            	<ul class="nav nav-treeview">            			
                  ...
                  <p>Create New LL</p>
						...
						<li class="nav-item">
                	<a href="admin-LL-manage.php" class="nav-link active">
                  <i class="far fa-circle nav-icon"></i>
                  <p>Manage LLs</p>
                	</a>
              		</li>	
...
<?php				
		if (isset($_POST['submitemail']))
	{
		...
		move_uploaded_file($_FILES["docu"]["tmp_name"], $path);
					$message = '
					<h3>This is an autogenerated email from BACS Quality LL Information System.</h3>
					</br>
				...
				if($mail->Send())								//Send an Email. Return true on success or false on error
				{
					...
					echo '</script>';
					//echo $message = "Congratulations! LL document has been sent to e-mail successfully.";						
				}		
     }
?>
	...
	<!--Content Wrapper. Contains Page Content-->
      ...
      <div class="container-fluid">
      	<div class="row mb-2">
         	<div class="col-sm-6">
            <h1 class="m-0 text-dark"> Review Lessons Learned Documents </h1>
          	</div><!--/.col-->
